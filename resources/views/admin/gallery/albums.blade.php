...
<div class="card">
  ...
  <div style="overflow-x:auto"><table class="data-table"><thead><tr><th>#</th><th>Album Title</th><th>Images</th><th>Status</th><th>Actions</th></tr></thead>
  <tbody>
    ...
    @empty<tr><td colspan="5" style="text-align:center;padding:40px;color:var(--text-muted)">No albums.</td></tr>@endforelse
  </tbody></table></div>
...
function viewR(id){crudView(`/admin/gallery/albums/${id}`,d=>`<div class="detail-grid">${detailRow('Title',d.title,true)}${detailRow('Status',d.status)}${detailRow('Images',d.images_count+' photos')}</div>`);}
function editR(id){crudEdit(`/admin/gallery/albums/${id}`,d=>{document.getElementById('eId').value=d.id;document.getElementById('eTitle').value=d.title;document.getElementById('eStatus').value=d.status;});}
function submitEdit(){const id=document.getElementById('eId').value;const fd=new FormData();fd.append('_method','PUT');fd.append('title',document.getElementById('eTitle').value);fd.append('status',document.getElementById('eStatus').value);post(`/admin/gallery/albums/${id}`,fd,'editModalError',()=>{closeModal('editModal');_reload();});}
function submitAdd(){const fd=new FormData();fd.append('title',document.getElementById('aTitle').value);fd.append('status',document.getElementById('aStatus').value);post('/admin/gallery/albums/store',fd,'addError',()=>{closeModal('addModal');_reload();});}
</script>
@endsection
